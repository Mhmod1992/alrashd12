            ...
            <table className="w-full text-sm text-right rtl:text-right text-slate-500 dark:text-slate-400">
                ...
                <tbody>
                    {requests.map((request) => {
                        ...
                        // Check if client has multiple requests (history) OR if they are an existing client (created before this request)
                        ...
                        // If client was created more than 2 minutes before the request, they are considered "existing/returning"
                        const isReturningClient = client?.created_at && (new Date(request.created_at).getTime() - new Date(client.created_at).getTime() > 120000); 
                        const hasHistory = clientRequestCount > 1 || isReturningClient;
