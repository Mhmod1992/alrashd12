...
const RequestFormBuilder: React.FC<RequestFormBuilderProps> = ({
  ...
  const {
    ...
    searchCarModels,
    checkCarHistory,
    updateRequestAndAssociatedData,
  ...
  const [paymentType, setPaymentType] = useState<PaymentType | "">(
    isReceptionist ? PaymentType.Unpaid : "",
  );
  ...
  const [unpaidDebtAlert, setUnpaidDebtAlert] = useState<
  ...
  const getInputClass = (fieldName: string) => {
    const base =
      "mt-1 block w-full p-3 border rounded-lg shadow-sm focus:ring-2 bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-200 transition-colors duration-200";
    if (errors[fieldName]) {
  ...
  const formInputClasses =
    "mt-1 block w-full p-3 border border-slate-300 dark:border-slate-600 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-200 transition-colors duration-200";
  ...
  const validateForm = () => {
    ...
    if (!clientName.trim()) {
      ...
      if (!firstInvalidField) {
        firstInvalidField = nameInputRef.current;
        invalidStep = 1;
      }
    }
    if (clientPhone.length < 10) {
      ...
      if (!firstInvalidField) {
        firstInvalidField = phoneInputRef.current;
        invalidStep = 1;
      }
    }
    ...
    if (useChassisNumber) {
      if (!chassisNumber.trim()) {
        ...
        if (!firstInvalidField) {
          firstInvalidField = chassisInputRef.current;
          invalidStep = 2;
        }
      }
    } else {
      if (!plateChars.trim()) {
        ...
        if (!firstInvalidField) {
          firstInvalidField = plateCharInputRef.current;
          invalidStep = 2;
        }
      }
      if (!plateNums.trim()) {
        ...
        if (!firstInvalidField) {
          firstInvalidField = plateNumInputRef.current;
          invalidStep = 2;
        }
      }
    }
    ...
    if (!carMakeSearchTerm.trim()) {
      ...
      if (!firstInvalidField) {
        firstInvalidField = makeInputRef.current;
        invalidStep = 2;
      }
    }
    if (!carModelSearchTerm.trim()) {
      ...
      if (!firstInvalidField) {
        firstInvalidField = modelInputRef.current;
        invalidStep = 2;
      }
    }
    if (!carYear) {
      ...
      if (!firstInvalidField) {
        firstInvalidField = yearInputRef.current;
        invalidStep = 2;
      }
    }
    ...
    if (!inspectionTypeId) {
      ...
      if (!firstInvalidField) {
        firstInvalidField = typeInputRef.current;
        invalidStep = 3;
      }
    }
    if (!inspectionPrice) {
      ...
      if (!firstInvalidField) {
        firstInvalidField = priceInputRef.current;
        invalidStep = 3;
      }
    }
    if (!isReceptionist && !paymentType) {
      ...
      if (!firstInvalidField) {
        firstInvalidField = paymentTypeRef.current;
        invalidStep = 3;
      }
    }
  ...
  useEffect(() => {
    ...
    if (debtCheckDebounceRef.current)
      clearTimeout(debtCheckDebounceRef.current);
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (
        ...
        !phoneInputRef.current.parentElement?.contains(event.target as Node)
      )
        setIsPhoneSuggestionsOpen(false);
      if (
        nameInputRef.current &&
        !nameInputRef.current.parentElement?.contains(event.target as Node)
      )
  ...
  const arabicTop = useMemo(
    () => convertToArabicNumerals(englishBottom),
    [englishBottom],
  );
  ...
  useEffect(() => {
    ...
    const hasPlate =
      !useChassisNumber &&
      plateChars.trim().length >= 3 &&
      plateNums.trim().length >= 1;
  ...
  const handleUseHistoryData = () => {
    ...
    addNotification({
      title: "تم التعبئة",
      message: "تم نسخ بيانات السيارة والعميل السابق.",
      type: "success",
    });
  ...
  const handleKeyDown = (
    ...
    if (e.key === "ArrowDown") {
      ...
      e.preventDefault();
      const itemToSelect =
        currentIndex >= 0 ? suggestions[currentIndex] : suggestions[0];
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (foundHistory) {
        ...
        carSnapshot = {
          make_ar: carMakeSearchTerm,
          make_en: carMakeSearchTerm,
          model_ar: carModelSearchTerm,
          model_en: carModelSearchTerm,
          year: carYear,
        };
      } else {
        ...
        const plateAr = useChassisNumber
          ? null
          : `${previewArabicChars.replace(/\s/g, "")} ${plateNums.replace(/\s/g, "")}`;
        const plateEn = useChassisNumber
          ? null
          : `${previewEnglishChars.replace(/\s/g, "")} ${plateNums.replace(/\s/g, "")}`;
      ...
      if (
        isEditMode ||
        finalPaymentType === PaymentType.Transfer ||
        finalPaymentType === PaymentType.Unpaid
      ) {
      ...
      const reqData = {
        ...
        broker:
          useBroker && brokerId
            ? { id: brokerId, commission: brokerCommission }
            : undefined,
        ...
        category_notes:
          isEditMode && initialData ? initialData.category_notes : {},
        structured_findings:
          isEditMode && initialData ? initialData.structured_findings : [],
        activity_log: isEditMode && initialData ? initialData.activity_log : [],
        attached_files:
          isEditMode && initialData ? initialData.attached_files : [],
      };
      ...
      if (isEditMode && initialData) {
        ...
        await updateRequestAndAssociatedData({
          ...
          formData: {
            client_id: clientId,
            car: {
              /* Update car details if changed logic needed here */
            },
            request: reqData,
          },
        });
        ...
        const newAddedRequest = await addRequestOptimized({
          ...
          carModelId: (
            await findOrCreateCarModel(
              carModelSearchTerm,
              (await findOrCreateCarMake(carMakeSearchTerm)).id,
            )
          ).id,
          carYear,
          plateNumber: useChassisNumber
            ? null
            : `${previewArabicChars.replace(/\s/g, "")} ${plateNums.replace(/\s/g, "")}`,
          plateNumberEn: useChassisNumber
            ? null
            : `${previewEnglishChars.replace(/\s/g, "")} ${plateNums.replace(/\s/g, "")}`,
          vin: useChassisNumber ? chassisNumber : null,
          carSnapshot,
      ...
      <div className={!isMobile ? "grid grid-cols-1 gap-6" : ""}>
        ...
        <div className={showStep(1) ? "block animate-fade-in" : "hidden"}>
          <fieldset
            ...
            className="bg-white dark:bg-slate-800/50 p-6 rounded-lg shadow-sm border dark:border-slate-700 h-full scroll-mt-20"
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="relative">
                ...
                <input
                  ...
                  onFocus={() =>
                    nameSuggestions.length > 0 && setIsNameSuggestionsOpen(true)
                  }
                  ...
                  <ul className="absolute z-10 w-full bg-white dark:bg-slate-800 border dark:border-slate-600 rounded-md mt-1 max-h-40 overflow-y-auto shadow-lg">
                    {nameSuggestions.map((c, idx) => (
                      <li
                        ...
                        className={`px-4 py-2 cursor-pointer text-sm ${idx === nameSuggestionIndex ? "bg-blue-100 dark:bg-slate-600" : "hover:bg-blue-50 dark:hover:bg-slate-700"}`}
                      >
                        {c.name}
                      </li>
                    ))}
                  ...
                  <input
                    ...
                    onKeyDown={(e) =>
                      handleKeyDown(
                        ...
                        phoneSuggestions,
                        isPhoneSuggestionsOpen,
                        setPhoneSuggestionIndex,
                        phoneSuggestionIndex,
                        handleClientSelection,
                        plateCharInputRef,
                      )
                    }
                  ...
                  <ul className="absolute z-10 w-full bg-white dark:bg-slate-800 border dark:border-slate-600 rounded-md mt-1 max-h-40 overflow-y-auto shadow-lg">
                    {phoneSuggestions.map((c, idx) => (
                      <li
                        key={c.id}
                        onClick={() => handleClientSelection(c)}
                        className={`px-4 py-2 cursor-pointer text-sm flex justify-between ${idx === phoneSuggestionIndex ? "bg-blue-100 dark:bg-slate-600" : "hover:bg-blue-50 dark:hover:bg-slate-700"}`}
                      >
                        <span>{c.phone}</span>{" "}
            ...
            <div className="mb-6">
              <div className="flex justify-between items-center mb-2">
                <label className="text-sm font-medium">
                  {useChassisNumber ? "رقم الشاصي (VIN)" : "رقم اللوحة"}
                </label>
                <div className="flex items-center">
                  <input
                    type="checkbox"
                    id="use-chassis"
                    checked={useChassisNumber}
                    onChange={() => setUseChassisNumber(!useChassisNumber)}
                    className="h-4 w-4 rounded border-slate-300 text-blue-600"
                  />
                  <label
                    htmlFor="use-chassis"
                    className="ms-2 text-xs text-slate-500"
                  >
                    استخدام رقم الشاصي
                  </label>
                </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-2 relative">
                    <input
                      ...
                      onBlur={() =>
                        !foundHistory && makeInputRef.current?.focus()
                      }
                ...
                <div className="relative">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setErrors((p) => ({ ...p, chassisNumber: false }));
                    }}
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <h4 className="font-bold text-amber-800 dark:text-amber-200">
                        هذه السيارة زارتنا من قبل
                      </h4>
                      <p className="text-sm text-amber-700 dark:text-amber-300 mt-1">
                        عدد الزيارات السابقة:{" "}
                        <strong>{foundHistory.previousRequests.length}</strong>
                      </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="relative" ref={makeDropdownRef}>
                <label className="flex justify-between text-sm font-medium mb-1">
                  ...
                  <button
                    type="button"
                    onClick={() => setIsCarScannerOpen(true)}
                ...
                {isMakeDropdownOpen && (
                  <ul className="absolute z-10 w-full bg-white dark:bg-slate-800 border dark:border-slate-600 rounded-md mt-1 max-h-60 overflow-y-auto shadow-lg">
                    {filteredMakes.map((m, idx) => (
                      <li
                        key={m.id}
                        onClick={() => handleMakeSelection(m)}
                        className={`px-4 py-2 cursor-pointer text-sm ${idx === makeSuggestionIndex ? "bg-blue-100 dark:bg-slate-600" : "hover:bg-blue-50 dark:hover:bg-slate-700"}`}
                      >
                        {m.name_en} - {m.name_ar}
                      </li>
                    ))}
                  ...
                  <ul className="absolute z-10 w-full bg-white dark:bg-slate-800 border dark:border-slate-600 rounded-md mt-1 max-h-60 overflow-y-auto shadow-lg">
                    {filteredModels.map((m, idx) => (
                      <li
                        ...
                        className={`px-4 py-2 cursor-pointer text-sm ${idx === modelSuggestionIndex ? "bg-blue-100 dark:bg-slate-600" : "hover:bg-blue-50 dark:hover:bg-slate-700"}`}
                      >
                        {m.name_en} - {m.name_ar}
                      </li>
                    ))}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setErrors((p) => ({ ...p, carYear: false }));
                  }}
                  className={getInputClass("carYear")}
                />
              </div>
            </div>
          </fieldset>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setErrors((p) => ({ ...p, inspectionType: false }));
                  }}
                  className={getInputClass("inspectionType")}
                >
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    setInspectionPrice(Number(e.target.value));
                    setErrors((p) => ({ ...p, inspectionPrice: false }));
                  }}
                  className={getInputClass("inspectionPrice")}
                />
              </div>
              {!isReceptionist && (
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      setErrors((p) => ({ ...p, paymentType: false }));
                    }}
                    ...
                    <div className="mt-2 p-2 bg-slate-100 dark:bg-slate-800 rounded text-sm grid grid-cols-2 gap-2">
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setSplitCardAmount(Number(inspectionPrice) - v);
                        }}
                  ...
                  {(isEditMode ||
                    ...
                    <div className="mt-2 text-sm">
                      <label className="block font-medium mb-1 line-clamp-1">
                        ملاحظة الدفع / الطلب
                      </label>
                      <input
                        type="text"
                        ...
                        className={getInputClass("paymentNote")}
                        placeholder="اختياري"
                      />
                    </div>
                  )}
        ...
        <div
          className={
            showStep(4) && !isReceptionist ? "block animate-fade-in" : "hidden"
          }
        >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        const b = brokers.find(
                          (br) => br.id === e.target.value,
                        );
                        setBrokerId(e.target.value);
                        if (b) setBrokerCommission(b.default_commission);
                      }}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setBrokerCommission(Number(e.target.value))
                    }
                    className={formInputClasses}
                  />
                </div>
        ...
        <div className="flex justify-between items-center gap-4">
          ...
          {currentStep < TOTAL_STEPS ? (
            <Button
              type="button"
              onClick={() => {
                if (validateForm()) setCurrentStep((p) => p + 1);
              }}
              className="flex-1 justify-center py-3 text-lg shadow-lg shadow-blue-500/20"
            >
              التالي{" "}
              <ChevronRightIcon className="w-5 h-5 ms-2 transform rotate-180" />
            </Button>
          ) : (
            <Button
              type="button"
              onClick={(e) => handleSubmit(e as any)}
              className="flex-1 justify-center py-3 font-bold text-lg shadow-lg shadow-blue-500/20"
            >
  ...
  return (
    <>
                ...
                return (
                  ...
                  >
                    ...
                    {index !== stepsList.length - 1 && (
                      <div
                        className={`absolute top-4 left-1/2 w-full h-[2px] -z-10 ${isCompleted ? "bg-green-500" : "bg-slate-200 dark:bg-slate-700"}`}
                      ></div>
                    )}
                    ...
                    <span
                      className={`text-[10px] mt-1 font-medium transition-colors ${isActive ? "text-blue-600 dark:text-blue-400" : isCompleted ? "text-green-600 dark:text-green-400" : "text-slate-400"}`}
                    >
                      {step.title}
                    </span>
                  </div>
      ...
      <CameraScannerModal
        ...
        onScanComplete={(data) => {
          setPlateChars(data.letters.split("").join(" "));
          setPlateNums(data.numbers.split("").join(" "));
      ...
      <CameraScannerModal
        ...
        onCarIdentify={(data) => {
          ...
          addNotification({
            title: "تم التعرف",
            message: `تم تحديد ${data.makeName} ${data.modelName}`,
            type: "success",
          });
        }}
        mode="car"
      ...
      >
        <div className="space-y-4">
          <input
            ...
            onChange={(e) =>
              setNewBrokerData((p) => ({ ...p, name: e.target.value }))
            }
            className={formInputClasses}
          />
          <input
            type="number"
            placeholder="العمولة"
            value={newBrokerData.default_commission}
            onChange={(e) =>
              setNewBrokerData((p) => ({
                ...p,
