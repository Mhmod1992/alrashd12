...
import { FinancialStats, InspectionRequest, Expense, Revenue, PaymentType, RequestStatus, ActivityLog, Client, Car } from '../types';
...
import { AreaChart, Area as RechartsArea, BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip as RechartsTooltip, ResponsiveContainer, Legend, LineChart as RechartsLineChart, Line, ReferenceLine, LabelList, ComposedChart } from 'recharts';
import { 
    PlusCircleIcon, 
    PenToolIcon, 
    ActivityIcon, 
...
}> = ({ onOpenSearchClient, onOpenVehicleHistorySearch, onOpenQuickExpense }) => {
...
const ExecutiveKpiCard: React.FC<{
    ...
    isLoading?: boolean;
}> = ({ title, value, subtitle, trend, icon, colorClass, bgClass, isLoading }) => {
    return (
...
const PerformanceCard: React.FC<{
    ...
    return (
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden flex flex-col h-full hover:shadow-md transition-shadow group">
            <div className={`p-2.5 border-b dark:border-slate-700 flex justify-between items-center ${headerBg}`}>
                <div className="flex items-center gap-2">
                    <span className="p-1 bg-white/50 dark:bg-black/20 rounded-lg text-slate-700 dark:text-white">
                        {icon}
                    </span>
                    <h4 className="font-bold text-slate-800 dark:text-slate-200 text-xs truncate">{title}</h4>
                </div>
            ...
            <div className="p-3 flex-1 overflow-y-auto custom-scrollbar bg-white/50 dark:bg-slate-800/50">
                {data.length > 0 ? (
                            ...
                            <div key={idx} className="flex justify-between items-center">
                                <div className="flex items-center gap-2">
                                    ...
                                    <div className="flex flex-col">
                                        <span className="text-[11px] font-bold text-slate-700 dark:text-slate-200">{item.name}</span>
                                        {item.subLabel && <span className="text-[9px] text-slate-400">{item.subLabel}</span>}
                                    </div>
                                </div>
                                <div className="flex items-center gap-2">
                                    <div className="flex flex-col items-end">
                                        <span className="text-xs font-black text-blue-600 dark:text-blue-400 font-numeric">{item.count}</span>
                                        <span className="text-[8px] text-slate-400 font-bold uppercase tracking-tighter">{mainCountLabel}</span>
                                    </div>
                    ...
                    <div className="h-full flex items-center justify-center">
                        <p className="text-slate-400 text-[10px] font-bold">لا يوجد بيانات</p>
                    </div>
                )}
            </div>
        </div>
...
const VehicleHistorySearchModal: React.FC<{ onClose: () => void }> = ({ onClose }) => {
    ...
    const [foundResult, setFoundResult] = useState<{ car: Car; previousRequests: InspectionRequest[]; lastClient?: Client } | null>(null);
    const [hasAttempted, setHasAttempted] = useState(false);
    const [carDetails, setCarDetails] = useState<{makeName?: string, modelName?: string}>({});
        ...
        try {
            ...
            if (result?.car) {
                ...
                if (!fetchedMakeName && result.car.make_id) {
                    const { data } = await supabase.from('car_makes').select('name_en, name_ar').eq('id', result.car.make_id).single();
                    if (data) fetchedMakeName = data.name_en || data.name_ar;
                }
                
                if (!fetchedModelName && result.car.model_id) {
                    const { data } = await supabase.from('car_models').select('name_en, name_ar').eq('id', result.car.model_id).single();
                    if (data) fetchedModelName = data.name_en || data.name_ar;
                }
                
                setCarDetails({ makeName: fetchedMakeName, modelName: fetchedModelName });
            }
        } catch (error) {
            ...
            <div className="space-y-6">
                <div className="bg-slate-50 dark:bg-slate-900/30 p-4 rounded-2xl border border-slate-100 dark:border-slate-800">
                    <div className="flex justify-between items-center mb-4">
                        <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider">بيانات التعريف</h4>
                        <div className="flex items-center gap-2">
                             <input 
                                type="checkbox" id="use-vin" 
                                checked={useVin} 
                                onChange={(e) => setUseVin(e.target.checked)} 
                                className="w-4 h-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500"
                            />
                            <label htmlFor="use-vin" className="text-xs font-bold text-slate-700 dark:text-slate-300 cursor-pointer">استخدام رقم الشاصي</label>
                        </div>
                    </div>
                            ...
                            <input
                                ...
                                onChange={(e) => {
                                    ...
                                    const validChars = rawVal.split('').filter(char => {
                                        return settings.plateCharacters.some(pc => 
                                ...
                                style={{ direction: /[\u0600-\u06FF]/.test(plateChars) ? 'rtl' : 'ltr' }}
                            />
                            <input
                                type="text"
                    ...
                    >
                        {isSearching ? <RefreshCwIcon className="w-4 h-4 animate-spin" /> : <SearchIcon className="w-4 h-4" />}
                        بحث عن المركبة
                    </Button>
                </div>
                    ...
                    <div className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
                        {foundResult ? (
                            <>
                                <div className="p-4 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800/50 rounded-2xl">
                                    <div className="flex flex-col items-center gap-4">
                                        <MiniPlateDisplay plateNumber={foundResult.car.plate_number || foundResult.car.vin || ''} settings={settings} />
                                        <div className="text-center">
                                            {(() => {
                                                const name = [carDetails.makeName, carDetails.modelName, foundResult.car.year].filter(Boolean).join(' ');
                                                return name ? <p className="font-bold text-emerald-900 dark:text-emerald-100 text-lg mb-1 uppercase tracking-wide">{name}</p> : null;
                                            })()}
                                            <p className="text-xs text-emerald-600 dark:text-emerald-400 font-bold">زارتنا {foundResult.previousRequests.length} مرة مسبقاً</p>
                                        </div>
                                    </div>
                                    
                                    {foundResult.lastClient && (
                                        <div className="mt-4 pt-4 border-t border-emerald-100 dark:border-emerald-800/50">
                                            <p className="text-[10px] text-emerald-500/70 uppercase font-black tracking-widest mb-1">آخر عميل مسجل</p>
                                            <div className="flex justify-between items-center">
                                                <span className="font-bold text-sm text-emerald-800 dark:text-emerald-200">{foundResult.lastClient.name}</span>
                                                <span className="text-xs text-emerald-600/80 font-mono italic">{foundResult.lastClient.phone}</span>
                                            </div>
                                        </div>
                                    )}
                                </div>
                                ...
                                <div className="space-y-2">
                                    <p className="text-xs font-bold text-slate-500 px-1">الزيارات الأخيرة</p>
                            ...
                            <div className="p-8 text-center border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-2xl">
                                ...
                                <p className="text-sm font-bold text-slate-500">لم يسبق لهذه المركبة زيارتنا من قبل</p>
                                <p className="text-[10px] text-slate-400 mt-1">يمكنك البدء بإنشاء طلب جديد لها</p>
                            </div>
                        )}
                    </div>
                )}
            ...
            <div className="space-y-4">
                ...
                <div className="max-h-64 overflow-y-auto custom-scrollbar">
                    {isLoading ? (
                        ...
                        <div className="space-y-2">
                            {results.map(client => (
                                <div key={client.id} className="p-3 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-lg hover:border-blue-300 transition-colors">
                                    <div className="flex justify-between items-start">
                                        <div>
                                            <p className="font-bold text-sm text-slate-700 dark:text-slate-200">{client.name}</p>
                                        ...
                                        <div className="bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 px-2 py-1 rounded text-xs font-bold items-center text-center">
                                            <div className="text-[10px] text-blue-400 dark:text-blue-500 mb-0.5">الطلبات</div>
                        ...
                        <p className="text-center text-slate-500 text-sm py-4">لا يوجد نتائج</p>
                    ) : null}
                </div>
...
// Quick Expense Modal
const QuickExpenseModal: React.FC<{ onClose: () => void; onSuccess?: () => void }> = ({ onClose, onSuccess }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await addExpense({
                ...
                employeeName: employeeData?.name || authUser?.email?.split('@')[0] || 'Unknown'
            });
...
const Dashboard: React.FC = () => {
  const { authUser, fetchServerFinancials, employees, technicians, clients, cars, carMakes, carModels, requests, reservations, systemLogs, fetchClientsCount, searchClients, addExpense, customFindingCategories } = useAppContext();
  ...
  const [clientStats, setClientStats] = useState<{ total: number, new: number, prevNew: number, returning?: number }>(() => {
      const cached = localStorage.getItem('dashboard_client_stats_cache');
      return cached ? JSON.parse(cached) : { total: 0, new: 0, prevNew: 0, returning: 0 };
  });
  ...
  const loadData = async () => {
      ...
      try {
          ...
          const [currentStats, previousStats, todayPulseStats, fullPulseStats, currentMonthStatsData, prevMonthStatsData, totalClientsCount, newClientsCount, prevNewClientsCount] = await Promise.all([
          ...
          const uniqueClientIdsForPeriod = new Set(currentStats.filteredRequests.map((r: any) => r.client_id).filter(Boolean));
          let returningClientsCount = Array.from(uniqueClientIdsForPeriod).filter(id => {
  ...
  // Growth Calculation
  const growthPercentage = clientStats.prevNew > 0 
      ? Math.round(((clientStats.new - clientStats.prevNew) / clientStats.prevNew) * 100) 
      : (clientStats.new > 0 ? 100 : 0);
  ...
  const performanceData = useMemo(() => {
      ...
      const topInspectorsByRole = Object.entries(techCategoryCounts).map(([role, techCounts]) => {
      ...
      const getListFromProcessed = (key: 'createdCount' | 'notedCount' | 'multipleSectionsCount') => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pb-24 p-2 sm:p-6 animate-fade-in custom-scrollbar">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6 mb-8 px-2">
            ...
            />
            <ExecutiveKpiCard 
                title={activePeriod === 'today' ? "عملاء جدد اليوم" : "عملاء جدد للفترة"} 
                value={`${clientStats.new.toLocaleString()}`} 
                subtitle={<div className="text-xs text-slate-500 font-bold">و {clientStats.returning || 0} عملاء قدامى</div>}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-6 gap-4 mb-8 px-2">
            <PerformanceCard 
                title="منشؤو الطلبات" 
                icon={<FilePlusIcon className="w-4 h-4 text-blue-600 dark:text-blue-400" />}
            ...
            <PerformanceCard 
                ...
                icon={<PenToolIcon className="w-4 h-4 text-purple-600 dark:text-purple-400" />}
            ...
            />
             <PerformanceCard 
                ...
                icon={<LayersIcon className="w-4 h-4 text-amber-600 dark:text-amber-400" />}
            ...
            {performanceData.inspectorsByRole.map((cat, idx) => (
                <PerformanceCard 
                    ...
                    icon={<SettingsIcon className="w-4 h-4 text-orange-600 dark:text-orange-400" />}
             ...
             <div className="lg:col-span-2 bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl rounded-[2rem] shadow-sm border border-white/20 dark:border-slate-700/50 p-6 relative overflow-hidden group hover:border-blue-300 dark:hover:border-blue-800 transition-all duration-500">
                 <div className="flex justify-between items-center mb-6 relative z-10">
                     <div>
                        ...
                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">مقارنة عدد الطلبات للشهر الحالي بالسابق</p>
                     </div>
                 </div>
                             ...
                             <ResponsiveContainer width="100%" height="100%">
                                 <BarChart data={monthlyRequestsComparisonData} margin={{ top: 20, right: 10, bottom: 5, left: 10 }}>
                                     ...
                                     <RechartsTooltip 
                                         ...
                                         formatter={(value: number, name: string) => [`${value.toLocaleString()} طلب`, name]}
                     ...
                     {isCarLoading ? <Skeleton className="w-full h-full rounded-2xl" /> : (
                         <div className="flex flex-col gap-3">
                             {carInspectionFrequencyData.length > 0 ? carInspectionFrequencyData.map((item, index) => {
                                 ...
                                 return (
                                     <div key={index} className="relative w-full h-11 bg-slate-50/50 dark:bg-slate-900/30 rounded-xl overflow-hidden flex items-center group/item border border-slate-100/50 dark:border-slate-700/30 shrink-0">
        ...
        {isVehicleHistoryModalOpen && (
            <VehicleHistorySearchModal onClose={() => setIsVehicleHistoryModalOpen(false)} />
        )}
        {isQuickExpenseModalOpen && (
            <QuickExpenseModal onClose={() => setIsQuickExpenseModalOpen(false)} onSuccess={() => loadData()} />
        )}
    </div>
  );
};
