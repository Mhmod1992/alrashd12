...
const CarsManagement: React.FC = () => {
    ...
    const handleExport = async () => {
        setIsExporting(true);
        try {
            addNotification({ title: 'جاري التصدير', message: 'يتم الآن تجميع بيانات السيارات والصور، يرجى الانتظار...', type: 'info' });

            // Fetch ALL models from the database to ensure we export everything, not just the currently loaded ones
            const { data: allModels, error: modelsError } = await supabase.from('car_models').select('*');
            if (modelsError) throw modelsError;

            const processedCarMakes = await Promise.all(carMakes.map(async (make) => {
                let logoBase64 = null;
                if (make.logo_url) {
            ...
            downloadAnchorNode.setAttribute("download", `cars_database_export_${new Date().toISOString().split('T')[0]}.json`);
    ...
    const handleImport = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            if (importedData.version !== '1.0' || importedData.type !== 'cars_database' || !importedData.data) {
                throw new Error('ملف غير صالح أو إصدار غير مدعوم.');
            }

            const { data } = importedData;

            setImportProgress('جاري استيراد ماركات وموديلات السيارات...');
            const makeIdMap: Record<string, string> = {}; 

            for (const make of data.carMakes || []) {
                let newLogoUrl = null;
                if (make.logoBase64) {
                    const imageFile = base64ToFile(make.logoBase64, `make_${Date.now()}.png`);
                    newLogoUrl = await uploadImage(imageFile, 'logos');
                }
            ...
            if (modelsToInsert.length > 0) {
                setImportProgress(`جاري استيراد ${modelsToInsert.length} موديل...`);
                // Insert in chunks to avoid payload size limits and improve performance
                const chunkSize = 500;
                for (let i = 0; i < modelsToInsert.length; i += chunkSize) {
                    const chunk = modelsToInsert.slice(i, i + chunkSize);
                    await addCarModelsBulk(chunk);
                }
            }
        ...
        <div className="space-y-4">
            <div className="flex justify-between items-center bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="font-bold text-slate-800 dark:text-white">قاعدة بيانات السيارات</h2>
                        <p className="text-xs text-slate-500 dark:text-slate-400">إدارة الشركات المصنعة والموديلات</p>
                    </div>
                </div>
